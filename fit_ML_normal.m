function result = fit_ML_normal( x,bins,hAx,plot_num )
...
%n(1)=[]; n(end)=[]; x_c(1)=[]; x_c(end)=[];
y       = sqrt(1/2/pi/sig2)*exp(-((x_c-u).^2)/(2*sig2));
RMS     = sqrt( (y-n)*((y-n)')/ (x_c(2)-x_c(1))^2 / (length(x_c)-1) );
mask    = y>0 & n>0;
d_KL_ny = sum(n(mask).*log(n(mask)./y(mask)));
d_KL_yn = sum(y(mask).*log(y(mask)./n(mask)));

% finish summarizing results
% ============================
result = struct( 'u',u,'sig2',sig2,'CRB_u',CRB_u,'CRB_sig2',CRB_sig2,'RMS',RMS,'d_KL_ny',d_KL_ny,'d_KL_yn',d_KL_yn,'type','ML' );

% plot distribution if asked for
% ===============================
plot(x_c,y,'r','LineWidth',2);
%if (nargin>1)
%    xspan = linspace(min(x),max(x),100);
%    if ishandle( hAx )
%        plot_normal( xspan,result,hAx,plot_num );
%    else
%        figure;
%        plot_normal( xspan,result,gca,plot_num );
%    end
%end