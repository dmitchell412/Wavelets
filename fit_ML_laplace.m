function result = fit_ML_laplace( x,bins,hAx,plot_num )
...
% ============================
result = struct( 'u',u,'b',b,'CRB_b',CRB_b,'RMS',RMS,'d_KL_ny',d_KL_ny,'d_KL_yn',d_KL_yn,'type','ML' );

% plot distribution if asked for
% ===============================
plot(x_c,y,'g','LineWidth',2);
%if (nargin>1)
%    xspan = linspace(min(x),max(x),100);
%    if ishandle( hAx )
%        plot_laplace( xspan,result,hAx,plot_num );
%    else
%        figure;
%        plot_laplace( xspan,result,gca,plot_num );
%    end
%end